...
class CrossBarSwitchNew(val width: Int, val sel_input: Boolean, val m: Int, val n: Int) extends Module {
  val io = IO(new Bundle {
    ...
    //val passOrReg = Vec(m, Input(Bool()))// To be registered or to pass-through?
  })
...
object CrossBarMainNew extends App {
  println("Hello World, I will now generate the Verilog file!")
  (new chisel3.stage.ChiselStage).emitVerilog(new CrossBarSwitchNew(64,true,8,8))
}
